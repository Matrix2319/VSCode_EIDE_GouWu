...
char zhiling[][20] = {
    ...
    "$DGT:4-4,1!",   // 推饮料    		                   3
    "$DGT:5-5,1!",   // 轻推饮料    		                4
    "$DGT:6-18,1!",  // 上中靠边抓饮料    		                5
    "$DGT:19-30,1!", // 下中抓  		                6
    "$DGT:31-41,1!", // 下右抓   		                7
    "$DGT:42-52,1!", // 下左抓    		                8
    "$DGT:84-95,1!", // 上左抓    		                9
    "$DGT:53-64!",  // 上右抓    		                10
    "$DGT:65-73,1!",  // 上中抓木块    		                11
    "$DGT:74-83,1!"  // 下中抓木块    		                12

};

// 方向0后退1前进2无3左4右5左移6右移7向后靠边8向前靠边
// 指令组下标为A,B,C,D对应各个区域，255为空,S为上，X为下，I为初始位置，1为轮盘复位
// 五个一组分别表示格数，方向，延时停车的延时时长ms，指令数组下标，等待机械臂延时s
unsigned char a[][5] = {

    {0, 5, 0, 255, 25},//D begin
    {0, 0, 30, 255, 0},
    {0, 0, 25, 255, 0},
    {0, 0, 8, 255, 0},
    {0, 6, 0, 255, 20},
    ...
    for (u8 i = 0; a[i][0] + a[i][1] + a[i][2] + a[i][3] + a[i][4] > 0; i++) {
        ...
        if (a[i][3] != 255) {
            ...
            } else if (a[i][3] == 'P') // 算法
            {
                ...
                for (u8 it = 0; it < a[i][4]; it++)
                    delay_ms(100);
            } else if (a[i][3] == 'r' || a[i][3] == 'g' || a[i][3] == 'b'||a[i][3] == 'j'||a[i][3] == 'h'||a[i][3] == 'z') {
    ...
    for (u8 i = 0; a[i][0] + a[i][1] + a[i][2] + a[i][3] + a[i][4] > 0; i++) {
        ...
        if (a[i][3] != 255) {
            ...
            } else if (a[i][3] == 'P') // 算法
            {
                ...
                for (u8 it = 0; it < a[i][4]; it++)
                    delay_ms(100);
            } else if (a[i][3] == 20||a[i][3] == 21||a[i][3] == 22||a[i][3] == 23||a[i][3] == 24||a[i][3] == 25) { // B区域推CD区域抓
                if (exFlag_HuoJia == 'B')
                    B_Tui(a[i][3]);
                else
                    KaoBian_Zhua(exFlag_HuoJia, a[i][3]);
            }
        }
        if (a[i][1] == 2 && a[i][4] != 0 && a[i][3] == 255) // 啥都不做时延时
        {
